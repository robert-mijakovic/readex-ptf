...
double Objective_Function_EDP1( int                  scenario_id,
    ...
    for( std::list<MetaProperty>::iterator iterator = base_properties.begin(), end = base_properties.end(); iterator != end; ++iterator ) {
        base_valueEDP1 *= ( iterator )->getSeverity();
    }

    for( std::list<MetaProperty>::iterator iterator = properties.begin(), end = properties.end(); iterator != end; ++iterator ) {
...
double Objective_Function_EDP2( int                  scenario_id,
                                ScenarioResultsPool* srp ) {
    std::list<MetaProperty> properties      = srp->getScenarioResultsByID( scenario_id );
    std::list<MetaProperty> base_properties = srp->getScenarioResultsByID( 0 );
    double                  EDP2            = 1.0;
...
double Objective_Function_EDP3( int                  scenario_id,
                                ScenarioResultsPool* srp ) {
    std::list<MetaProperty> properties      = srp->getScenarioResultsByID( scenario_id );
    std::list<MetaProperty> base_properties = srp->getScenarioResultsByID( 0 );
    double                  EDP3            = 1.0;
...
PCAPPlugin::PCAPPlugin() : app( Application::instance() ) {
}
 ...
 */
void PCAPPlugin::parse_opts( int   argc,
                             char* argv[] ) {
    optind = 1;
    while( optind < argc ) {
        int index = -1;
...
void PCAPPlugin::parseConfigFile() {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to parseConfigFile()\n" );
    ...
    if( inputFile.is_open() ) {
        ...
        while( getline( inputFile, line ) ) {
            ...
            if( key == "search_algorithm" ) {
                ...
                if( value == "gde3" ) {
                    ...
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: search algorithm to be loaded changed to gde3\n" );
                }
                else if( value == "individual" ) {
                    searchAlgorithmName = "individual";
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: search algorithm to be loaded changed to individual\n" );
                }
                else if( value == "random" ) {
                    searchAlgorithmName = "random";
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: search algorithm to be loaded changed to random\n" );
                }
                else if( value == "exhaustive" ) {
                    searchAlgorithmName = "exhaustive";
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: search algorithm to be loaded changed to exhaustive\n" );
                }
                else {
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Invalid options for search algorithm. Valid options are \"exhaustive\",\"gde3\",\"individual\" & \"random\".\n" );
                }
            }
            else if( key == "parallelRegions" ) {
                if( value == "true" ) {
                    withParallelRegions = true;
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: parallel regions will be used for tuning\n" );
                }
                else if( value == "false" ) {
                    withParallelRegions = false;
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: parallel regions will NOT be used for tuning\n" );
                }
                else {
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Invalid options for parallel regions. Valid options are \"true\" or \"false\"\n" );
                }
            }
            else if( key == "userRegion" ) {
                if( value == "true" ) {
                    withUserRegion = true;
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: User region will be used for tuning\n" );
                }
                else if( value == "false" ) {
                    withUserRegion = false;
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: User region will NOT be used for tuning\n" );
                }
                else {
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Invalid options for user regions. Valid options are \"true\" or \"false\"\n" );
                }
            }
                ...
                try{
                    ...
                    if( noRegions > 0 ) {
                        ...
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: At least %d parallel regions will be considered for tuning\n", minNoRegions );
                    }
                    else {
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Invalid option for minRegionsToConsider. Valid option is a integer value > 0\n", minNoRegions );
                    }
                }
                ...
                try{
                    ...
                    if( noRegions > 0 ) {
                        ...
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: At most %d parallel regions will be considered for tuning\n", maxNoRegions );
                    }
                    else {
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Invalid option for maxRegionsToConsider. Valid option is a integer value > 0\n", minNoRegions );
                    }
                }
                catch( const boost::bad_lexical_cast& ) {
                    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "Invalid option for noRegionsToConsider\n" );
                ...
                try{
                    ...
                    if( cutoff > 0 && cutoff < 1 ) {
                        firstCutOff = cutoff;
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: firstCutOff set to %.2lf.\n", firstCutOff );
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: parallel regions with exec time >= %.2lf will be considered for tuning until max limit is reached.\n", firstCutOff );
                    }
                    else {
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Invalid value for cutoff1. Valid option is a float value between 0 and 1.\n" );
                    }
                }
                catch( const boost::bad_lexical_cast& ) {
                ...
                try{
                    ...
                    if( cutoff > 0 && cutoff < 1 ) {
                        secondCutOff = cutoff;
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: secondCutOff set to %.2lf.\n", secondCutOff );
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: parallel regions with exec time >= %.2lf will be considered depending on number of regions already considered.\n", secondCutOff );
                    }
                    else {
                        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Invalid value for cutoff2. Valid option is a float value between 0 and 1.\n" );
                    }
                }
                catch( const boost::bad_lexical_cast& ) {
        ...
        if( maxNoRegions < minNoRegions ) {
            psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Maximum no of regions (%d) cannot be less than minimum no of regions (%d).\n", maxNoRegions, minNoRegions );
            maxNoRegions = minNoRegions;
            psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Setting maximum no of regions to be equal as minimum no of regions (%d).\n", minNoRegions );
        }

        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: completed reading the configuration file.\n" );
    }
    else {
        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: could not open configuration file.\n" );
    }
}
 ...
 *
 */
void PCAPPlugin::initialize( DriverContext*   context,
    ...
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Before exiting to initialize, string is: %s\n\n", results.c_str() );
...
void PCAPPlugin::createScenarios( void ) {
    ...
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to createScenarios() completed\n" );
}
...
void PCAPPlugin::defineExperiment( int               numprocs,
 ...
 * false otherwise.
 */
bool PCAPPlugin::restartRequired( std::string& env,
                                  int&         numprocs,
                                  std::string& command,
...
bool PCAPPlugin::searchFinished( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to searchFinished()\n" );
...
bool PCAPPlugin::tuningFinished( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to tuningFinished()\n" );
...
Advice* PCAPPlugin::getAdvice( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to getAdvice()\n" );
...
void PCAPPlugin::curveFitting( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to curveFitting()\n" );
...
void PCAPPlugin::finalize() {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to finalize()\n" );
    terminate();
}
...
void PCAPPlugin::terminate() {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to terminate()\n" );
...
IPlugin* getPluginInstance( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to getComponentInstance()\n" );
...
int getVersionMajor( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to getInterfaceVersionMajor()\n" );
...
int getVersionMinor( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to getInterfaceVersionMinor()\n" );
 ...
 */
string getName( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to getName()\n" );
...
string getShortSummary( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to getShortSummary()\n" );
...
void PCAPPlugin::startTuningStep2EnergyTuning( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to startTuningStep()\n" );
    ...
    if( selected_search != NULL ) {
        ...
        setObjectiveFunction( Objective_Function_EDP1 );
    }
...
void PCAPPlugin::createScenarios2EnergyTuning( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to createScenarios()\n" );
    ...
    if( firstTuningStep ) {
        ...
        if( withParallelRegions ) {
            ...
            for( int i = 0; i < regionExectimeMap.size(); i++ ) {
                ...
                addRegionToSearchAlgorithm( selected_region, regionExectimeMap[ i ].second );
            ...
            if( noRegionsAdded == 0 ) {
                for( int i = 0; i < regionExectimeMap.size(); i++ ) {
                    ...
                    addRegionToSearchAlgorithm( selected_region, regionExectimeMap[ 0 ].second );
                    if( noRegionsAdded == minNoRegions ) {
                        break;
                    }
                }
...
void PCAPPlugin::prepareScenarios2EnergyTuning( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to prepareScenarios()\n" );
...
void PCAPPlugin::defineExperiment2EnergyTuning( int               numprocs,
                                                bool&             analysisRequired,
                                                StrategyRequest** strategy ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to defineExperiment()\n" );
...
//    ts->front()->setSingleRank(i);
    ...
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: Added %d scenario in the experiment.\n", i );
...
void PCAPPlugin::finishTuningStep2EnergyTuning( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "PCAPPlugin: call to processResults()\n" );
...
void PCAPPlugin::writePCAPresults() {
    ...
    result_oss << "\nAll Results:\n";
    result_oss << "Scenario | Threads                      | EDP        | EDP2        | EDP^3        | Energy        | Execution Time\n";
    result_file << "Scenario ID; Threads;EDP;EDP2;EDP^3;Energy;Time\n";
    for( int scenario_id = 0; scenario_id < Num_Scenarios; scenario_id++ ) {
        scenario = ( *pool_set->fsp->getScenarios() )[ scenario_id ];
        ...
        for( ts_iter = scenario->getTuningSpecifications()->begin(); ts_iter != scenario->getTuningSpecifications()->end(); ts_iter++ ) {
            ...
            for( iter = ( *ts_iter )->getVariant()->getValue().begin(); iter != ( *ts_iter )->getVariant()->getValue().end(); iter++ ) {
                nthr = iter->second;
            }
    ...
    results_file.open( "pcap_results.txt" );
