...
Scenario::Scenario( Region*                     region_in,
    ...
    tuned_region = region_in  ?  region_in->getRegionID() : "";
    ts           = ts_in;
    pr           = pr_in;
}
...
Region* Scenario::getRegion() {
    if( tuned_region.empty() ) {
        return NULL;
    }

    return Application::instance().getRegionByID( tuned_region );
}
...
void Scenario::setTunedRegion( const std::string& id ) {
    tuned_region = id;
}
...
void Scenario::print( void ) {
    ...
    if( !tuned_region.empty() ) {
        ...
        printf( " No tuned region specified.\n" );
        fflush( stdout );
    }
    ...
    if( ts ) {
        ...
        for( ts_iter tsi = ts->begin(); tsi != ts->end(); tsi++, ts_count++ ) {
            ...
            if( ( *tsi )->getTypeOfVariantContext() == variant_context_type( PROGRAM ) ) {
                ...
                list<string>*  regions = context.context_union.region_list;
                printf( "REGION_LIST of size %" PRIu64 "\n", regions->size() );
                fflush( stdout );
                std::list<string>::iterator region_iter;
