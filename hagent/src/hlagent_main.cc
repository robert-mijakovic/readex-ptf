...
struct cmdline_opts {
    ...
    char info_string[ 2000 ];
    char selectiveinfo_string[ 4000 ];
...
//      { const char* name,         int has_arg,       int* flag, int val }
        ...
        { "selective-info",         required_argument, 0,         'e' },
...
//TODO: Revise unused stuff. tag seems to be used only by HL and AA. -RM
    ...
    while( optind < argc ) {
        ...
        case 'd':
            if( opt->has_arg == required_argument ) {
                copts->has_info = 1;
                strcpy( copts->info_string, optarg );
            }
            break;

        case 'e':
            if( opt->has_arg == required_argument ) {
                copts->has_selectiveinfo = 1;
                strcpy( copts->selectiveinfo_string, optarg );
...
void usage( int   argc,
    ...
    fprintf( stderr, "  [--selective-info=list]  (Individual info level names separated by comma)\n" );
    print_dbgLevelsDefs();
...
//  msg_prefix << "HLA(" << getpid() << ")";
    ...
    // ----------
    if( opts.has_info ) {
        psc_dbg_level = atoi( opts.info_string );
    }
    else {
        char  buf[ 800 ];
        int   dbg_level;
