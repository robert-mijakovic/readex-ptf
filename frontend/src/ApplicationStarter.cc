...
void ApplicationStarter::runAnalysisAgent( AgentDetails* agent ) {
    ...
    common_command << " --info=" << psc_dbg_level;

    if( opts.has_inst ) {
        common_command << " --inst=" << opts.inst_string;
    }

    if( opts.has_selectiveinfo ) {
        common_command << " --selective-info=" << opts.selectiveinfo_string;
    }
...
void ApplicationStarter::runHlAgent( AgentDetails* agent ) {
    ...
    command_string <<  " --info="  <<  psc_dbg_level;
    if( opts.has_selectiveinfo )
        command_string <<  " --selective-info="  <<  opts.selectiveinfo_string;
